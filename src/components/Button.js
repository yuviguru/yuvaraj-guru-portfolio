...
export default function Button({ text, icon, disabled = false, className = "" }) {
    // Check if this is the loading state (spinning icon)
    ...
    return (
        ...
        >
            ...
            <span className={`absolute right-0 bottom-0 w-[49px] h-[49px] bg-primary rounded-full text-typography flex items-center justify-center group-hover:bg-white transition-colors duration-300`}>
                <FontAwesomeIcon
                    icon={icon}
                    size="lg"
